...
public class ConstraintGraphClass {
	...
	public EdgeStructClass ConstructGraph(dataStructure ds, List<ObsClass> obsList, Map<String, String> labelDic, ConstraintClass cc) {
		...
		for (int k = 0; k < consList.size(); k++) {
			...
			if (factorName.equals("StrungWeight_StrungWeight_SameValue")) {
				Set<String> edges = Link_StrungWeight_StrungWeight_SameValue(ds.data, obsList);
				edgeStruct.MapEdgeToConstraints(edges, factorName);
				set.addAll(edges);
			} else if (factorName.equals("StrungWeight_Larger_UnstrungWeight")) {
				Set<String> edges = Link_StrungWeight_Larger_UnstrungWeight(ds.data, obsList);
				edgeStruct.MapEdgeToConstraints(edges, factorName);
				set.addAll(edges);
			}
	...
	public EdgeStructClass ConstructGraph(dataStructure ds, List<ObsClass> obsList, Map<String, String> labelDic, ConstraintClass cc, metaParaClass mpc, InferClass inferP) {
		...
		for (int k = 0; k < consList.size(); k++) {
			...
			if (factorName.equals("StrungWeight_StrungWeight_SameValue")) {
				int stateID = Integer.valueOf(labelDic.get("StrungWeight"));
				Set<String> edges = Link_StrungWeight_StrungWeight_SameValue(ds.data, obsList, stateID, mpc.eps.get("StrungWeight_StrungWeight_SameValue"), inferP);
				edgeStruct.MapEdgeToConstraints(edges, factorName);
				set.addAll(edges);
			} else if (factorName.equals("StrungWeight_Larger_UnstrungWeight")) {
				String state1 = "StrungWeight";
				int stateID1 = Integer.valueOf(labelDic.get(state1));
				String state2 = "UnstrungWeight";
				int stateID2 = Integer.valueOf(labelDic.get(state2));
				Set<String> edges = Link_StrungWeight_Larger_UnstrungWeight(ds.data, obsList, stateID1, stateID2, mpc.eps.get("StrungWeight_Larger_UnstrungWeight"), inferP);
				edgeStruct.MapEdgeToConstraints(edges, factorName);
				set.addAll(edges);
			}
	...
	private Set<String> Link_StrungWeight_StrungWeight_SameValue(List<LineClass2> data, List<ObsClass> obsList) {
	...
	private Set<String> Link_StrungWeight_StrungWeight_SameValue(List<LineClass2> data, List<ObsClass> obsList, int stateID, double eps, InferClass inferP) {
		Set<String> adj = new HashSet<String>();
	...
	private Set<String> Link_StrungWeight_Larger_UnstrungWeight(List<LineClass2> data, List<ObsClass> obsList) {
		Set<String> adj = new HashSet<String>();
		for (int i = 0; i < data.size() - 1; i++) {
			ObsClass obs1 = obsList.get(i);
			//LineClass2 lc1 = data.get(i);
			for (int j = i + 1; j < data.size(); j++) {
				...
				if (obs1.HasWordWeight && obs2.HasWordWeight) {
					String edge = i + "_" + j; // i=StrungWeight, j=UnstrungWeight, remark: only need to generate an edge once and each edge (i,j) has i<j
					adj.add(edge);
				}
			}
		}
	...
	private Set<String> Link_StrungWeight_Larger_UnstrungWeight(List<LineClass2> data, List<ObsClass> obsList, int stateID1, int stateID2, double eps, InferClass inferP) {
		Set<String> adj = new HashSet<String>();
		for (int i = 0; i < data.size() - 1; i++) {
			ObsClass obs1 = obsList.get(i);
			//LineClass2 lc1 = data.get(i);
			for (int j = i + 1; j < data.size(); j++) {
				...
				if (obs1.HasWordWeight && obs2.HasWordWeight) {
					if (Math.log(inferP.nodeBel[i][stateID1]) +  Math.log(inferP.nodeBel[j][stateID2]) >= eps) {
						String edge = i + "_" + j; // i=StrungWeight, j=UnstrungWeight
						adj.add(edge);
					} else if (Math.log(inferP.nodeBel[j][stateID1]) +  Math.log(inferP.nodeBel[i][stateID2]) >= eps) {
						String edge = i + "_" + j; // j=StrungWeight, i=UnstrungWeight
						adj.add(edge);
					}
				}
			}
