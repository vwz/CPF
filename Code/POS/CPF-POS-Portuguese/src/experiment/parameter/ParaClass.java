...
public class ParaClass {
	...
	// --------------------------------------------------------------------
	// this is for computing gradient from sufficient statistics
	...
	/*public void Minus(SuffStatsClass other) {
		...
		for (Entry<String, double[]> e : w.entrySet()) {
			double[] src = e.getValue();
			List<double[]> tar = other.w.get(e.getKey());
			double[] value = f_minus(src, tar);
			w.put(e.getKey(), value);
		}
	}*/
	...
	public void InitializeByRandom(int nStates, int nWords, int nFeatures, ConstraintClass cc, Map<String, Double> b) {
		...
		// for v
		String factorName = "Label_Label";
		double[] para = SetByRandom(nStates * nStates, r);
		v.put(factorName, para);

		/*String factorName = "Label_Label_Feature";
		double[] para = SetByRandom(nStates * nStates * nFeatures, r);
		v.put(factorName, para);
		
		factorName = "Label_Label_Token";
		para = SetByRandom(nStates * nStates * nWords, r);
		v.put(factorName, para);*/
		...
		for (int i = 0; i < cc.constraints.size(); i++) {
			...
			double value = 5 * conf;
			//double value = r.nextDouble() * 0.1 * 30 * conf;
