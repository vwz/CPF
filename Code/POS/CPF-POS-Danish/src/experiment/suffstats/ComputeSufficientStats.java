...
public class ComputeSufficientStats {
			...
			if (n1 == i) {
				...
				if (nextStateID > -1 && stateID > -1) {
					...
					for (int k = 0; k < lc.wordIDList.size(); k++) { 
						...
						index = im.GetIndexStateStateToken(stateID, nextStateID, wordID); 
						value = suffstats.v.get(factorName);
						double[] record = new double[] {index, p * 1.0}; 
						value.add(record);
						suffstats.v.put(factorName, value);
					...
					for (int featureID = 0; featureID < lc.features.length; featureID++) {
						if (lc.features[featureID] > 0) {
							index = im.GetIndexStateStateFeature(stateID, nextStateID, featureID); 
							value = suffstats.v.get(factorName);
							double[] record = new double[] {index, p * 1.0}; 
							value.add(record);
							suffstats.v.put(factorName, value);
					...
					for (int featureID = 0; featureID < lc.next_features.length; featureID++) {
						if (lc.next_features[featureID] > 0) {
							index = im.GetIndexStateStateFeature(stateID, nextStateID, lc.features.length + featureID); 
							value = suffstats.v.get(factorName);
							double[] record = new double[] {index, p * 1.0}; 
							value.add(record);
							suffstats.v.put(factorName, value);
				...
				if (prevStateID > -1 && stateID > -1) {
					...
					/*factorName = "Label_Label_Token";
					for (int k = 0; k < other.wordIDList.size(); k++) { 
						if (other.wordTypeList.get(k).startsWith("pos_")) continue;
						if (other.wordTypeList.get(k).startsWith("prev_")) continue;
						//if (other.wordTypeList.get(k).startsWith("prev2gram_")) continue;

						int wordID = Integer.valueOf(other.wordIDList.get(k)).intValue();
						index = im.GetIndexStateStateToken(prevStateID, stateID, wordID); 
						value = suffstats.v.get(factorName);
						double[] record = new double[] {index, p * 1.0}; 
						value.add(record);
						suffstats.v.put(factorName, value);
					...
					factorName = "Label_Label_Feature";
					for (int featureID = 0; featureID < other.features.length; featureID++) {
						if (other.features[featureID] > 0) {
							index = im.GetIndexStateStateFeature(prevStateID, stateID, featureID); 
							value = suffstats.v.get(factorName);
							double[] record = new double[] {index, p * 1.0}; 
							value.add(record);
							suffstats.v.put(factorName, value);
					...
					factorName = "Label_Label_Feature";
					for (int featureID = 0; featureID < other.next_features.length; featureID++) {
						if (other.next_features[featureID] > 0) {
							index = im.GetIndexStateStateFeature(prevStateID, stateID, other.features.length + featureID); 
							value = suffstats.v.get(factorName);
							double[] record = new double[] {index, p * 1.0}; 
							value.add(record);
							suffstats.v.put(factorName, value);
	...
	public void GetEdgeSuffStatsForV(dataStructure ds, int n1, int n2, String yprop1, String yprop2, SuffStatsClass suffstats, double p) {
		...
		/*factorName = "Label_Label_Token";
		for (int k = 0; k < lc1.wordIDList.size(); k++) { 
			if (lc1.wordTypeList.get(k).startsWith("pos_")) continue;
			if (lc1.wordTypeList.get(k).startsWith("prev_")) continue;
			//if (lc1.wordTypeList.get(k).startsWith("prev2gram_")) continue;

			int wordID = Integer.valueOf(lc1.wordIDList.get(k)).intValue();
			index = im.GetIndexStateStateToken(stateID1, stateID2, wordID); 
			value = suffstats.v.get(factorName);
			double[] record = new double[] {index, p * 1.0}; 
			value.add(record);
			suffstats.v.put(factorName, value);
		...
		factorName = "Label_Label_Feature";
		for (int featureID = 0; featureID < lc1.features.length; featureID++) {
			if (lc1.features[featureID] > 0) {
		...
		factorName = "Label_Label_Feature";
		for (int featureID = 0; featureID < lc1.next_features.length; featureID++) {
			if (lc1.next_features[featureID] > 0) {
				index = im.GetIndexStateStateFeature(stateID1, stateID2, lc1.features.length + featureID); 
				value = suffstats.v.get(factorName);
				double[] record = new double[] {index, p * 1.0}; 
				value.add(record);
				suffstats.v.put(factorName, value);
	...
	public void GetNodeSuffStatsForV_Ytype_alpha2(dataStructure ds, int i, String yprop, List<int[]> blanketedges, SuffStatsClass suffstats, List<ObsClass> obsList, ConstraintClass cc, metaParaClass mpc, double p, double q, List<double[]> p_edges, List<double[]> q_edges, Map<String, Double> Eqzeta) {
		...
		for (int k = 0; k < consList.size(); k++) {
			...
			for (int j = 0; j < lc1.wordIDList.size(); j++) {
				...
				double count = lc1.countList.get(j);
				if (count == 0) continue;
			...
			factorName = "Label_Features";
			for (int j = 0; j < lc1.next_features.length; j++) {
				double count = lc1.next_features[j];
				if (count == 0) continue;

				index = im.GetIndexStateFeatures(stateID, lc1.features.length + j);
				value = suffstats.v.get(factorName);
				record = new double[] {index, e * q * (f_value - b * g_value) * sigmaterm * (1 - p) * count};
				value.add(record);
				suffstats.v.put(factorName, value);
			...
			for (int j = 0; j < blanketedges.size(); j++) {
				...
				if (i == edge[0]) {
					for (int l = 0; l < nStates; l++) {
						for (int m = 0; m < lc1.wordIDList.size(); m++) { 
							if (lc1.wordTypeList.get(m).startsWith("pos_")) continue;
							if (lc1.wordTypeList.get(m).startsWith("prev_")) continue;

							int wordID = Integer.valueOf(lc1.wordIDList.get(m)).intValue();
							index = im.GetIndexStateStateToken(stateID, l, wordID);
					...
					LineClass2 lc2 = variables.get(edge[0]);

					for (int l = 0; l < nStates; l++) {
						for (int m = 0; m < lc2.wordIDList.size(); m++) { 
							if (lc2.wordTypeList.get(m).startsWith("pos_")) continue;
							if (lc2.wordTypeList.get(m).startsWith("prev_")) continue;

							int wordID = Integer.valueOf(lc2.wordIDList.get(m)).intValue();
							index = im.GetIndexStateStateToken(l, stateID, wordID);
			...
			for (int j = 0; j < blanketedges.size(); j++) {
				...
				if (i == edge[0]) {
					for (int l = 0; l < nStates; l++) {
						for (int featureID = 0; featureID < lc1.features.length; featureID++) {
							if (lc1.features[featureID] > 0) {
								index = im.GetIndexStateStateFeature(stateID, l, featureID); 
								value = suffstats.v.get(factorName);
								record = new double[] {index, e * q_edgeBel[l] * (f_value - b * g_value) * sigmaterm * (1 - p_edgeBel[l]) * 1.0}; 
								value.add(record);
								suffstats.v.put(factorName, value);
						...
						for (int featureID = 0; featureID < lc1.next_features.length; featureID++) {
							if (lc1.next_features[featureID] > 0) {
								index = im.GetIndexStateStateFeature(stateID, l, lc1.features.length + featureID); 
								value = suffstats.v.get(factorName);
								record = new double[] {index, e * q_edgeBel[l] * (f_value - b * g_value) * sigmaterm * (1 - p_edgeBel[l]) * 1.0};
								value.add(record);
								suffstats.v.put(factorName, value);
					...
					for (int l = 0; l < nStates; l++) {
						for (int featureID = 0; featureID < lc2.features.length; featureID++) {
							if (lc2.features[featureID] > 0) {
								index = im.GetIndexStateStateFeature(l, stateID, featureID); 
								value = suffstats.v.get(factorName);
								record = new double[] {index, e * q_edgeBel[l] * (f_value - b * g_value) * sigmaterm * (1 - p_edgeBel[l]) * 1.0}; 
								value.add(record);
								suffstats.v.put(factorName, value);
						...
						for (int featureID = 0; featureID < lc2.next_features.length; featureID++) {
							if (lc2.next_features[featureID] > 0) {
								index = im.GetIndexStateStateFeature(l, stateID, lc2.features.length + featureID); 
								value = suffstats.v.get(factorName);
								record = new double[] {index, e * q_edgeBel[l] * (f_value - b * g_value) * sigmaterm * (1 - p_edgeBel[l]) * 1.0};
								value.add(record);
								suffstats.v.put(factorName, value);
	...
	public void GetEdgeSuffStatsForV_Ytype_alpha2(dataStructure ds, int n1, int n2, String yprop1, String yprop2, List<int[]> blanketedges, SuffStatsClass suffstats, List<ObsClass> obsList, ConstraintClass cc, List<String> consList, metaParaClass mpc, double p, double q, double[] p_nodes, double[] q_nodes, List<double[]> p_edges, List<double[]> q_edges, Map<String, Double> Eqzeta, int indexp) {
		...
		for (int k = 0; k < consList.size(); k++) {
			...
			for (int j = 0; j < lc2.wordIDList.size(); j++) {
				...
				double count = lc2.countList.get(j);
			...
			for (int j = 0; j < lc1.next_features.length; j++) {
				...
				index = im.GetIndexStateFeatures(stateID1, lc1.features.length + j);
				value = suffstats.v.get(factorName);
				record = new double[] {index, e * q_nodes[0] * (f_value - b * g_value) * sigmaterm * (1 - p_nodes[0]) * count};
				value.add(record);
				suffstats.v.put(factorName, value);
			...
			for (int j = 0; j < lc2.next_features.length; j++) {
				double count = lc2.next_features[j];
				if (count == 0) continue;

				index = im.GetIndexStateFeatures(stateID2, lc2.features.length + j);
				value = suffstats.v.get(factorName);
				record = new double[] {index, e * q_nodes[1] * (f_value - b * g_value) * sigmaterm * (1 - p_nodes[1]) * count};
				value.add(record);
				suffstats.v.put(factorName, value);
				...
				for (int l = 0; l < nStates; l++) {
					if (n1 == edge[0]) {
						for (int m = 0; m < lc1.wordIDList.size(); m++) { 
							...
							int wordID = Integer.valueOf(lc1.wordIDList.get(m)).intValue();
							index = im.GetIndexStateStateToken(stateID1, l, wordID);
						...
						LineClass2 other = ds.data.get(edge[0]);

						for (int m = 0; m < other.wordIDList.size(); m++) { 
							if (other.wordTypeList.get(m).startsWith("pos_")) continue;
							if (other.wordTypeList.get(m).startsWith("prev_")) continue;

							int wordID = Integer.valueOf(other.wordIDList.get(m)).intValue();
							index = im.GetIndexStateStateToken(l, stateID1, wordID);
						...
						for (int m = 0; m < lc2.wordIDList.size(); m++) { 
							...
							int wordID = Integer.valueOf(lc2.wordIDList.get(m)).intValue();
							index = im.GetIndexStateStateToken(stateID2, l, wordID);
						...
						for (int m = 0; m < other.wordIDList.size(); m++) { 
							...
							int wordID = Integer.valueOf(other.wordIDList.get(m)).intValue();
							index = im.GetIndexStateStateToken(l, stateID2, wordID);
			...
			for (int j = 0; j < blanketedges.size(); j++) {
				...
				for (int l = 0; l < nStates; l++) {
					if (n1 == edge[0]) {
						for (int featureID = 0; featureID < lc1.features.length; featureID++) {
							if (lc1.features[featureID] > 0) {
								index = im.GetIndexStateStateFeature(stateID1, l, featureID); 
								value = suffstats.v.get(factorName);
								record = new double[] {index, e * q_edgeBel[l] * (f_value - b * g_value) * sigmaterm * (1 - p_edgeBel[l]) * 1.0}; 
								value.add(record);
								suffstats.v.put(factorName, value);
						...
						for (int featureID = 0; featureID < lc1.next_features.length; featureID++) {
							if (lc1.next_features[featureID] > 0) {
								index = im.GetIndexStateStateFeature(stateID1, l, lc1.features.length + featureID); 
								value = suffstats.v.get(factorName);
								record = new double[] {index, e * q_edgeBel[l] * (f_value - b * g_value) * sigmaterm * (1 - p_edgeBel[l]) * 1.0};
								value.add(record);
								suffstats.v.put(factorName, value);
						...
						for (int featureID = 0; featureID < other.features.length; featureID++) {
							if (other.features[featureID] > 0) {
								index = im.GetIndexStateStateFeature(l, stateID1, featureID); 
								value = suffstats.v.get(factorName);
								record = new double[] {index, e * q_edgeBel[l] * (f_value - b * g_value) * sigmaterm * (1 - p_edgeBel[l]) * 1.0}; 
								value.add(record);
								suffstats.v.put(factorName, value);
						...
						for (int featureID = 0; featureID < other.next_features.length; featureID++) {
							if (other.next_features[featureID] > 0) {
								index = im.GetIndexStateStateFeature(l, stateID1, other.features.length + featureID); 
								value = suffstats.v.get(factorName);
								record = new double[] {index, e * q_edgeBel[l] * (f_value - b * g_value) * sigmaterm * (1 - p_edgeBel[l]) * 1.0};
								value.add(record);
								suffstats.v.put(factorName, value);
						...
						for (int featureID = 0; featureID < lc2.features.length; featureID++) {
							if (lc2.features[featureID] > 0) {
								index = im.GetIndexStateStateFeature(stateID2, l, featureID); 
								value = suffstats.v.get(factorName);
								record = new double[] {index, e * q_edgeBel[l] * (f_value - b * g_value) * sigmaterm * (1 - p_edgeBel[l]) * 1.0}; 
								value.add(record);
								suffstats.v.put(factorName, value);
						...
						for (int featureID = 0; featureID < lc2.next_features.length; featureID++) {
							if (lc2.next_features[featureID] > 0) {
								index = im.GetIndexStateStateFeature(stateID2, l, lc2.features.length + featureID); 
								value = suffstats.v.get(factorName);
								record = new double[] {index, e * q_edgeBel[l] * (f_value - b * g_value) * sigmaterm * (1 - p_edgeBel[l]) * 1.0};
								value.add(record);
								suffstats.v.put(factorName, value);
						...
						for (int featureID = 0; featureID < other.features.length; featureID++) {
							if (other.features[featureID] > 0) {
								index = im.GetIndexStateStateFeature(l, stateID2, featureID); 
								value = suffstats.v.get(factorName);
								record = new double[] {index, e * q_edgeBel[l] * (f_value - b * g_value) * sigmaterm * (1 - p_edgeBel[l]) * 1.0}; 
								value.add(record);
								suffstats.v.put(factorName, value);
						...
						for (int featureID = 0; featureID < other.next_features.length; featureID++) {
							if (other.next_features[featureID] > 0) {
								index = im.GetIndexStateStateFeature(l, stateID2, other.features.length + featureID); 
								value = suffstats.v.get(factorName);
								record = new double[] {index, e * q_edgeBel[l] * (f_value - b * g_value) * sigmaterm * (1 - p_edgeBel[l]) * 1.0};
								value.add(record);
								suffstats.v.put(factorName, value);
